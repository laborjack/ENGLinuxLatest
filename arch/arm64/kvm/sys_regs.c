...
static void reset_mpidr(struct kvm_vcpu *vcpu, const struct sys_reg_desc *r)
{
	 ...
	 */
	vcpu_sys_reg(vcpu, MPIDR_EL1) = (1 << 31) | (vcpu->vcpu_id & 0xff);
}
...
static int fpsimd_get_reg(const struct kvm_vcpu *vcpu, u64 id, void __user *uaddr)
{
	...
	if (regid < num_fpsimd_data_regs()) {
		if (KVM_REG_SIZE(id) != 16)
			return -ENOENT;
		return reg_to_user(uaddr, &vcpu_gp_regs(vcpu)->fp_regs.vregs[regid],
				   id);
	}
	...
	case KVM_REG_ARM64_FP_SIMD_FPSR:
		return reg_to_user(uaddr, &vcpu_gp_regs(vcpu)->fp_regs.fpsr, id);
	case KVM_REG_ARM64_FP_SIMD_FPCR:
		return reg_to_user(uaddr, &vcpu_gp_regs(vcpu)->fp_regs.fpcr, id);
	default:
		return -ENOENT;
	}
}
...
static int fpsimd_set_reg(struct kvm_vcpu *vcpu, u64 id, const void __user *uaddr)
{
	...
	if (regid < num_fpsimd_data_regs()) {
		if (KVM_REG_SIZE(id) != 16)
			return -ENOENT;
		return reg_from_user(&vcpu_gp_regs(vcpu)->fp_regs.vregs[regid],
				     uaddr, id);
	}
	...
	switch (regid) {
	case KVM_REG_ARM64_FP_SIMD_FPSR:
		return reg_from_user(&vcpu_gp_regs(vcpu)->fp_regs.fpsr, uaddr, id);
	case KVM_REG_ARM64_FP_SIMD_FPCR:
		return reg_from_user(&vcpu_gp_regs(vcpu)->fp_regs.fpcr, uaddr, id);
	default:
		return -ENOENT;
	}
}
